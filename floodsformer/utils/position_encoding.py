# Based on: https://github.com/XiYe20/VPTR/blob/main/utils/position_encoding.py

import torch
from torch import nn
import math
from floodsformer.utils.misc import NestedTensor
...
class PositionEmbeddding2D(nn.Module):
    ...
    https://github.com/facebookresearch/detr/blob/master/models/position_encoding.py
    """
    def __init__(self, temperature=10000, normalize=False, scale=None, device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')):
        super().__init__()
        self.temperature = temperature
        self.normalize = normalize
        self.device = device
...
class PositionEmbeddding3D(nn.Module):
    ...
    """
    def __init__(self, E, T, temperature=10000, normalize=False, scale=None, device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')):
        """
