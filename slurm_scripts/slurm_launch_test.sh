...
#SBATCH --job-name=FloodSformer_test
#SBATCH --output=%x.o%j
#SBATCH --error=%x.e%j
#SBATCH --nodes=1
...
cd ${WORKINGDIR}
python3 ${WORKINGDIR}/run_test.py \
        --cfg ${WORKINGDIR}/configs/Parflood/DB_Parma_20m_testFS.yaml  # CHANGE: use the path to the file .yaml with the appropriate parameters (see README.md)
